...
export default function EditSchedulePage() {
  ...
  const fetchData = async () => {
    try {
      const [schedule, details, allWorkouts] = await Promise.all([
        supabaseApi.getSchedule(params.id),
        supabaseApi.getScheduleDetails(params.id),
      ...
      if (details) {
        const mappedWorkouts = details.map((detail, index) => ({
          id: detail.id || index,
          day: detail.day,
          workout_id: detail.workout_id,
          workout_name: detail.workout?.name || 'Unknown',
          sets: detail.workout?.sets || 0,
          reps: detail.workout?.reps || 0,
        }));
        setScheduleWorkouts(mappedWorkouts);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await supabaseApi.updateSchedule(params.id, scheduleData);
      
      // Delete old details and add new ones
      await supabaseApi.deleteScheduleDetails(params.id);
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6">
          <div className="mb-6">
            ...
            <p className="text-gray-600 mt-1">Update schedule details</p>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6 max-w-6xl">
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex space-x-4">
                ...
                >
                  <Save size={20} />
                  <span>{saving ? 'Updating...' : 'Update Schedule'}</span>
                </button>
                <Link
                  href="/schedules"
